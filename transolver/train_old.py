...
def main(device, train_dataset, val_dataset, Net, hparams, path, criterion='MSE', reg=1, val_iter=10,
    ...
    for epoch in pbar_train:
        ...
        for data in train_dataset:
            ...
            # print('data_sampled.x.size(0): ',data_sampled.x.size(0))
            # print('hparams[subsampling]',hparams['subsampling'])
            idx = random.sample(range(data_sampled.x.size(0)), data_sampled.x.size(0))
            ...
            if name_mod != 'PointNet' and name_mod != 'MLP':
                ...
                data_sampled.edge_index = data_sampled.edge_index                                        

            train_dataset_sampled.append(data_sampled)
        train_loader = DataLoader(train_dataset_sampled, batch_size=hparams['batch_size'], shuffle=True)
        del (train_dataset_sampled)

        train_loss, _ = train(device, model, train_loader, optimizer,
