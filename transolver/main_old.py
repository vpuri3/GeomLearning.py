...
file_path= os.path.join(args.my_path, 'processed_set.pt')
dataset = torch.load(file_path)[0:10]
dataset_train = dataset[0:8]
# test_dataset = manifest[args.task + '_test'] if args.task != 'scarce' else manifest['full_test']
test_dataset = dataset[-2:]
# n = int(.1 * len(dataset_train))
train_dataset = dataset_train[0:8]
val_dataset = dataset_train[0:8]
...
# n = int(.1 * len(dataset_train))
# train_dataset = dataset_train[:-n]
# val_dataset = dataset_train[-n:]
...
for i in range(args.nmodel):
    ...
    print('end training')
    models.append(model)
# torch.save(models[0], osp.join(args.save_path, args.task, args.model, args.model))
torch.save(models[0].state_dict(), osp.join(args.save_path, args.task, args.model, args.model))
