...
def eval_model(model, data, data_n, norm_coeffs, device="cpu"):
    ''' 
    ...
    '''
    try:
        pred_n = model(data_n.to(device)).cpu().detach().numpy()
        # print('pred_n',pred_n)
        pred = unnormalize(pred_n,norm_coeffs[2].numpy(),norm_coeffs[3].numpy())
        # print('pred: ',pred)
        # print('pred',pred)
...
def eval_model_all(model, dataset, dataset_n, norm_coeffs, device="cpu"):
    vals = []
    for i in range(len(dataset)):
        vals.append(eval_model(model, dataset[i], dataset_n[i], norm_coeffs, device=device))
    # for data in dataset:
    #     vals.append(eval_model(model, data, data_n, device=device))
    vals = np.array(vals)
    return vals

def eval_model_multiple(model, datasets, datasets_n, norm_coeffs, device="cpu"):
    vals = dict()
    for key in datasets:
        vals[key] = eval_model_all(model, datasets[key], datasets_n[key], norm_coeffs, device=device)
    return vals
